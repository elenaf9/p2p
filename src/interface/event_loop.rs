    ...
    TRq: FwRequest<Rq>,
{
    ...
    /// - Poll the commands-channel for [`SwarmCommand`]s that are sent from `Network`.
    ///
    /// If all `Network` clones are dropped, the command-channel will return `None` and `EventLoop` will shut
    /// down.
