...
use p2p::{firewall::FirewallRules, ChannelSinkConfig, EventChannel, Network, NetworkBuilder, ReceiveRequest};
...
async fn init_peer() -> (mpsc::Receiver<ReceiveRequest<Ping, Pong>>, Network<Ping, Pong>) {
    let (dummy_tx, _) = mpsc::channel(10);
    let (request_channel, rq_rx) = EventChannel::new(10, ChannelSinkConfig::BufferLatest);

    let builder = NetworkBuilder::new(dummy_tx, request_channel, None, FirewallRules::allow_all())
